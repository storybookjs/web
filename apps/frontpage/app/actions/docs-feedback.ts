...
/* eslint-disable no-console -- This is a server file */
...
const { trickyHeaderKey, trickyHeaderValue } =
  // prettier-ignore
  /^key-(?<trickyHeaderKey>.+)-value-(?<trickyHeaderValue>.+)$/.exec(trickyHeader)?.groups || {};
...
export async function queryGitHub<D extends object>(
  query: string,
  { variables = {} } = {},
): Promise<D> {
  let response;
...
export async function submitFeedback({
  ...
  try {
    ...
    if (!hasValidTrickyHeader || !hasValidOrigin || !hasValidReferer || spuriousComment) {
      console.info(
        JSON.stringify(
          {
    ...
    const hasValidRating = rating && Object.keys(ratingSymbols).includes(rating);
